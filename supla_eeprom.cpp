...
void save_DS18b20_name(String save, int nr) {
  int start = 1 + MAX_SSID + MAX_PASSWORD + MAX_MLOGIN + MAX_MPASSWORD + MAX_SUPLA_SERVER + MAX_SUPLA_ID + MAX_SUPLA_PASS + MAX_HOSTNAME + (SUPLA_GUID_SIZE * 2) + MAX_BUTTON + MAX_RELAY +  MAX_RELAY_STATE +  (MAX_DS18B20_SIZE * MAX_DS18B20_ARR) + (MAX_DS18B20_NAME * nr);
...
void save_max_ds18b20(uint8_t val) {
  int start = 1 + MAX_SSID + MAX_PASSWORD + MAX_MLOGIN + MAX_MPASSWORD + MAX_SUPLA_SERVER + MAX_SUPLA_ID + MAX_SUPLA_PASS + MAX_HOSTNAME + (SUPLA_GUID_SIZE * 2) + MAX_BUTTON + MAX_RELAY +  MAX_RELAY_STATE +  (MAX_DS18B20_SIZE * MAX_DS18B20_ARR) + (MAX_DS18B20_NAME * MAX_DS18B20_ARR);
  EEPROM.begin(EEPROM_SIZE);
  EEPROM.put(start, val);
  EEPROM.commit();
  EEPROM.end();
}
...
uint8_t read_max_ds18b20() {
  ...
  EEPROM.begin(EEPROM_SIZE);
  delay(100);
  EEPROM.get(start, max_ds);
  EEPROM.end();

  if (max_ds >= MAX_DS18B20_ARR) max_ds = MAX_DS18B20_ARR; else if (max_ds == 0) max_ds = 1;

  return max_ds;
}

void save_bme_elevation(int temp) {
  int start = 1 + MAX_SSID + MAX_PASSWORD + MAX_MLOGIN + MAX_MPASSWORD + MAX_SUPLA_SERVER + MAX_SUPLA_ID + MAX_SUPLA_PASS + MAX_HOSTNAME + (SUPLA_GUID_SIZE * 2) + MAX_BUTTON + MAX_RELAY +  MAX_RELAY_STATE +  (MAX_DS18B20_SIZE * MAX_DS18B20_ARR) + (MAX_DS18B20_NAME * MAX_DS18B20_ARR) + MAX_MAX_DS18B20;
  EEPROM.begin(EEPROM_SIZE);
...
void save_supla_button_duration(int nr, int duration) {
  Serial.print("zapisano duration "); Serial.println(duration);
  int start = 1 + MAX_SSID + MAX_PASSWORD + MAX_MLOGIN + MAX_MPASSWORD + MAX_SUPLA_SERVER + MAX_SUPLA_ID + MAX_SUPLA_PASS + MAX_HOSTNAME + (SUPLA_GUID_SIZE * 2) + MAX_BUTTON + MAX_RELAY +  MAX_RELAY_STATE +  (MAX_DS18B20_SIZE * MAX_DS18B20_ARR) + (MAX_DS18B20_NAME * MAX_DS18B20_ARR)  + MAX_MAX_DS18B20 + MAX_BME_ELEVATION;
  EEPROM.begin(EEPROM_SIZE);
